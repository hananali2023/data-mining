...
def similarity(item1, item2, item_user_dict, item_user_rating_dict, item_avg_rating, similarity):
...
def predict_rating(user, item, user_item_dict, item_user_dict, item_user_rating_dict, user_avg_rating, item_avg_rating, sim_cache):
    ...
    for rated_item in user_item_dict[user]:
        similar = similarity(item, rated_item, item_user_dict, item_user_rating_dict, item_avg_rating, sim_cache)
        if similar > 0:
            rated_item_rating = item_user_rating_dict[rated_item][user]
            similarities.append((similar, rated_item_rating))
...
if __name__ == '__main__':
    ...
    similarity = {}
    predictions = test_pairs.map(lambda x: (x[0], x[1], predict_rating(x[0], x[1], user_item_dict, item_user_dict, item_user_rating_dict, user_avg_rating, item_avg_rating, similarity))).collect()
