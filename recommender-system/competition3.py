...
# add features from the json files provided to lower the RSME. Features such as the average of useful, funny, and cool and
# years users have been Yelping in user.json. Then, I ran an XGBoost regression model to train the features to predict user
...
def load_data(sc, folder_path):
    review_rdd = sc.textFile(f"{folder_path}/review_train.json").map(json.loads)
    ...
    checkin_rdd = sc.textFile(f"{folder_path}/checkin.json").map(json.loads)
    tip_rdd = sc.textFile(f"{folder_path}/tip.json").map(json.loads)
    photo_rdd = sc.textFile(f"{folder_path}/photo.json").map(json.loads)

    user_tip_counts = tip_rdd.map(lambda x: (x['user_id'], 1)).reduceByKey(lambda a, b: a + b).collectAsMap()
    business_text_review_counts = review_rdd.map(lambda x: (x['business_id'], 1)).reduceByKey(lambda a, b: a + b).collectAsMap()
    user_text_review_counts = review_rdd.map(lambda x: (x['user_id'], 1)).reduceByKey(lambda a, b: a + b).collectAsMap()
    checkin_counts = checkin_rdd.map(lambda x: (x['business_id'], sum(x['time'].values()))).collectAsMap()
    photo_counts = photo_rdd.map(lambda x: (x['business_id'], 1)).reduceByKey(lambda a, b: a + b).collectAsMap()
    tip_counts = tip_rdd.map(lambda x: (x['business_id'], 1)).reduceByKey(lambda a, b: a + b).collectAsMap()

    return review_rdd, user_rdd, business_rdd, checkin_rdd, checkin_counts, photo_rdd, photo_counts, tip_rdd, tip_counts, user_tip_counts, business_text_review_counts, user_text_review_counts



def extract_features(user_rdd, business_rdd, user_tip_counts, business_text_review_counts, user_text_review_counts, checkin_counts, photo_counts, tip_counts):
    current_year = datetime.now().year

    user_features = user_rdd.map(lambda u: (u['user_id'], (u.get('review_count', 0), u.get('average_stars', 0.0),
            u.get('fans', 0),
            current_year - int(u['yelping_since'].split('-')[0]),
            1 if u.get('elite') != "None" else 0,
            u['useful'] + u['funny'] + u['cool'],
            sum([u['compliment_hot'], u['compliment_cool'], u['compliment_more'], u['compliment_writer'], u['compliment_photos']]),
            user_tip_counts.get(u['user_id'], 0), user_text_review_counts.get(u['user_id'], 0),
            (u['useful'] + u['funny'] + u['cool']) / max(1, u['review_count']))))

    business_features = business_rdd.map(lambda b: (b['business_id'], (b.get('review_count', 0), b.get('stars', 0.0),
    ...
    business_text_review_counts_rdd = sc.parallelize(business_text_review_counts.items())

    business_features = business_features.leftOuterJoin(business_text_review_counts_rdd).mapValues(lambda x: x[0] + (x[1] or 0,))
...
if __name__ == "__main__":
    ...
    (review_rdd, user_rdd, business_rdd, checkin_rdd, checkin_counts, photo_rdd, photo_counts, tip_rdd, tip_counts, user_tip_counts, business_text_review_counts, user_text_review_counts) = load_data(sc, folder_path)
    ...
    user_features, business_features = extract_features(user_rdd, business_rdd, user_tip_counts, business_text_review_counts, user_text_review_counts,
        checkin_counts, photo_counts, tip_counts)
