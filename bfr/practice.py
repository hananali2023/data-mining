...
def calculate_mahalanobis(point, cluster_summary):
    ...
    return math.sqrt( float( delta.T.dot(inv_cov).dot(delta) ) )
...
def assign_points(points, cluster_summaries, distance_threshold):
    ...
    for index, point in points.items():
        ...
        if nearest_cluster is not None:
            ...
            cluster_summaries[nearest_cluster]["POINTS"].append(index)
        else:
    ...
    while merged:
        ...
        for i in range(len(cs_keys)):
            for j in range(i + 1, len(cs_keys)):
                ...
                centroid2 = np.array(sum2["SUM"]) / sum2["N"]
                dist1 = calculate_mahalanobis(centroid1, sum2)
                dist2 = calculate_mahalanobis(centroid2, sum1)
                if max(dist1, dist2) < distance_threshold:
                    sum1["N"] += sum2["N"]
                    sum1["SUM"] += sum2["SUM"]
                    sum1["SUMSQ"] += sum2["SUMSQ"]
                    sum1["POINTS"].extend(sum2["POINTS"])
                    del compression_set[c2]
                    merged = True
                    break
            if merged:
                break
...
def process_data_chunk(chunk, discard_set, compression_set, retained_set, num_clusters, points_map, next_cluster_id):
    points = chunk
    ...
    if len(retained_set) >= max(2 * num_clusters, 10):
        ...
        for cluster_id, members in rs_clusters.items():
            if len(members) > 1:
                compression_set[next_cluster_id] = summarize_clusters({next_cluster_id: members}, points_map)[next_cluster_id]
                for member in members:
                    retained_set.discard(member)
                next_cluster_id += 1
        compression_set = merge_cs_clusters(compression_set, distance_threshold)
    return discard_set, compression_set, retained_set, next_cluster_id


def bfr(input_file, num_clusters, output_file):
    global sc
    data_rdd = sc.textFile(input_file).map(lambda line: line.split(',')).map(
        lambda line: (int(line[0]), list(map(float, line[1:]))))
    points_map = data_rdd.collectAsMap()
    point_items = list(points_map.items())
    chunks = [dict() for _ in range(5)]
    for idx, (gid, vec) in enumerate(point_items):
        chunks[idx % 5][gid] = vec
    ...
    for i in range(5):
        chunk = chunks[i]
        if i == 0:
            initial_k = 5 * num_clusters
            tmp_kmeans = KMeans(n_clusters=initial_k, random_state=0).fit(list(chunk.values()))
            to_RS = set()
            for lbl in set(tmp_kmeans.labels_):
                pos = [j for j, L in enumerate(tmp_kmeans.labels_) if L == lbl]
                if len(pos) < small_cluster:
                    for j in pos:
                        to_RS.add(list(chunk.keys())[j])
            RS = {idx: chunk[idx] for idx in to_RS}
            ...
            discard_set, compression_set, retained_set, next_cluster_id = process_data_chunk(
                chunk, discard_set, compression_set, retained_set, num_clusters, points_map, next_cluster_id)
    ...
    ds_final = sum(cluster["N"] for cluster in discard_set.values())
    total_pts = len(points_map)
    print(f"{ds_final} {total_pts}")
